...
@section('content')
   ...
   <!-- Product Create start -->
    ...
    <div class="whitebg radius5 p-3" >
      <h4>Edit Product <br> <strong class="mainColor" > {{$product->title}} </strong> </h4>
    </div>
   ...
   <div class="whitebg radius5 p-3 mt-3 " >
        ...
        <input class="inputCommon" value="{{$product->title}}" name="title" type="text">
        @error('title')
        ...
        <input class="inputCommon" value="{{$product->price}}" name="price" type="number">  
        @error('price')
        ...
        <div class="row">
          ...
          <label> Category</label>
            ...
            <option value="{{$parent->id}}"    @if($product->category !=null)
              @if($product->category->name == $parent->name)
           selected
             @endif
          @endif >{{$parent->name}}</option>
           ...
           <option value="{{$child->id}}" @if($product->category !=null)
...
<label>Place</label>
  <select class="commonSelect" name="place" id="">
    <option value="">Select a place</option>
     <option value="trandy" @if($product->place == "trandy")   selected @endif >Trandy</option>
     <option value="popular" @if($product->place == "popular")   selected @endif >Popular</option>
     <option value="other" @if($product->palce == "other")   selected @endif  >other</option>
        ...
        <div class="row">
          <div class="col-md-6">
           ...
           <select class="commonSelect" name="offer" id="">
              <option value="0" @if($product->offer == 0) selected @endif >OFF</option>
              <option value="1"  @if($product->offer == 1) selected @endif >ON</option>
          ...
          <div class="col-md-6">
            ...
            <input class="inputCommon" value="{{$product->offerprice}}" name="offerprice" type="number">
 ...
 @endforeach
        ...
        <textarea class="commontextarea" name="des" id="editor" rows="5">{{$product->description}}</textarea>
          
        <label>Slug</label>
        <input class="inputCommon" value="{{$product->slug}}" name="slug" type="text">
        @error('slug')
   ...
   <input type="submit" class="theme-btn mt-3" value="Update product"  >
